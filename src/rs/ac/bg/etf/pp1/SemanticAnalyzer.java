...
public class SemanticAnalyzer extends VisitorAdaptor {
	...
	public void visit(MethodDeclStart methodDeclStart) {
		...
		*/
		...
		/*
		TODO ovo dodati nakon definicije metode

		if (this.getCurrScope().equals(Scope.CLASS)
				) {
			Obj superMethod = Tab.currentScope().findSymbol(identifier);
			superMethod.
			// TODO updateuj super metodu ako treba ako je ima (ako je ova sad overriduje)
			// todo kada procitac celu metodu u klasnom opsegu, ako on overrajduje, samo nadji tu metodu koju overajduje i setLocals(this.getLocals)
		}
		else

		*/

		Obj currMethodObj = new Obj(Obj.Meth, identifier, this.currType,0,0);//Tab.insert(Obj.Meth, identifier, this.currType);
